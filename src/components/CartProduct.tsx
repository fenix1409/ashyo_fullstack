...
const CartProduct: FC<{ item: CartProductType }> = ({ item }) => {
    ...
    const [_, setBasketQuantity] = useState<ProductItemType[]>([])
    ...
    const handleDecrement = () => {
        setBasketQuantity(prevState =>
            prevState.map(product =>
                product.product_id === item.product.product_id && (product.quantity || 1) > 1
        ...
        <div className='flex justify-between gap-[31px]'>
            ...
            <div className='py-[17px] '>
                <div className='flex items-center gap-[10px] mb-[51px]'>
                    <strong className='font-bold text-[24px]'>{item.product.price}</strong> <span className='text-[14px]'>USZ</span>
                </div>
